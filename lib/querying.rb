...
def select_value_and_count_of_most_prolific_species
  "SELECT characters.species, MAX(spec_count) FROM characters GROUP BY characters.species HAVING (SELECT spec_count FROM characters COUNT(characters.species) as spec_count)"
end
...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters ON series.id = characters.series_id GROUP BY series.id HAVING characters.species = 'human'"
end
